...
先安装并登录WorkBuddy，等待环境准备完成，结束任务并退出软件（Windows包括托盘）。
...
Windows、Apple Silicon Mac、Intel Mac各111项回归通过：[三平台结果](https://github.com/JachinLan/jjaitech-memory/actions/runs/35503288170)。公开命令专项结果见发布页。员工账号桌面验收仍需执行。

GitHub无法访问时，可用发布页同一份经校验ZIP离线分发代码；离线包不含运行环境。详见[完整安装手册](docs/INSTALLATION.md)。

## 双平台安装

请按发布页选择Windows PowerShell或Mac终端入口。优先复用WorkBuddy自带环境；支持已校验的代码ZIP离线分发。步骤、依赖、策略和限制见[安装手册](docs/INSTALLATION.md)。
...
MCP为Python标准库stdio进程，不监听端口。安装器配置本地Bash启动器，在原Python路径失效时寻找其他可用本地运行环境。Windows持久记录Git Bash路径；新安装会验证实际注册、MCP及Raw，失败恢复配置与原运行缓存。详见docs/INSTALLATION.md。
